...
class ZoteroEntries:
    ...
    def sioyek_get_hash(self, zotero_key):
        conn = sqlite3.connect(self._sioyek_db_local)
        cursor = conn.cursor()

        query = f"SELECT hash FROM document_hash WHERE path LIKE '%{zotero_key}%'"
    ...
    def sioyek_get_highlights(self, zotero_key):
        hash = self.sioyek_get_hash(zotero_key)
        if not hash:
            return []
        print(hash)
        conn = sqlite3.connect(self._sioyek_db_shared)
        cursor = conn.cursor()
        query = "SELECT desc, text_annot, type, creation_time, modification_time FROM highlights WHERE document_path = ?"
        cursor.execute(query, (hash,))
        highlights = cursor.fetchall()

        conn.close()

        return [f"{a}\n > {b}" for a, b in highlights] 
    ...
    def _copy_zotero_data(self, dbpath, dbname):
        ...
        zcopy = self._tmpdir + f"/{dbname}.sqlite"

        zcopy_time = os.path.getmtime(zcopy) if os.path.isfile(zcopy) else 0
    ...
    def GetAnnotations(self, citekey, offset=0):
